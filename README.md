...
| 2 | `2_train_siglip2_from_delta.py` | Fine-tune SigLIP2, log to MLflow, register UC model | **AI Runtime (Serverless GPU)** |
...
3. **Notebook 2: Training** — Open the notebook, select **Serverless GPU** as compute, choose **A10** as the accelerator in the Environment panel, and run.
...
You can combine all 4 notebooks into a single Job with task dependencies: `prepare_data → train → deploy → batch_inference`. Only Notebook 2 requires AI Runtime (GPU); the rest run on Serverless Notebook.

**Via Jobs API / Databricks Asset Bundles:**

```yaml
tasks:
  - task_key: prepare_data
    notebook_task:
      notebook_path: /Workspace/.../1_prepare_training_data
      source: WORKSPACE
  ...
  - task_key: deploy_model
    depends_on:
      - task_key: train_siglip2
    notebook_task:
      notebook_path: /Workspace/.../3_deploy_model
      source: WORKSPACE
  ...
  - task_key: batch_inference
    depends_on:
      - task_key: deploy_model
    notebook_task:
      notebook_path: /Workspace/.../4_batch_inference
      source: WORKSPACE
...
| 2 | `2_train_siglip2_from_delta.py` | SigLIP2 ファインチューニング・MLflow 記録・UC モデル登録 | **AI Runtime (Serverless GPU)** |
...
2. **ノートブック 1: データ準備**（Serverless Notebook）

   ノートブックを開き、コンピュートで **Serverless** を選択して実行します。STAIR Captions のダウンロードと Delta テーブルへの登録を行います。

3. **ノートブック 2: トレーニング**（AI Runtime / Serverless GPU）

   ノートブックを開き、コンピュートで **Serverless GPU** を選択します。Environment パネルでアクセラレータに **A10** を指定して実行してください。

4. **ノートブック 3: デプロイ**（Serverless Notebook）

   Serverless Notebook で実行します。GPU サービングエンドポイントが作成されます。初回起動に最大 60 分かかる場合があります。

5. **ノートブック 4: バッチ推論**（Serverless Notebook）

   エンドポイントが READY になったら、Serverless Notebook で実行します。`ai_query()` で画像を一括処理し、結果を Delta テーブルに保存します。

#### Job として実行する場合

4つのノートブックを依存関係付きの1つの Job にまとめて実行できます。ノートブック 2 のみ AI Runtime（GPU）を使用し、残りは Serverless Notebook で実行されます。

**Databricks UI から作成する場合:**

1. Workflows → Create Job
2. 以下の4つのタスクを追加し、依存関係を `prepare_data → train → deploy → batch_inference` の順に設定
   - タスク 1, 3, 4: Compute Type = **Serverless**
   - タスク 2: Compute Type = **Serverless**、Hardware Accelerator = **GPU_1xA10**、Environment = dependencies に `transformers==4.51.3`, `pynvml` を追加

**Jobs API / Databricks Asset Bundles で作成する場合:**
